...
sub _data_cmd
{
 ...
 if(${*$ftp}{'net_ftp_passive'} &&
     ...
     !defined ${*$ftp}{'net_ftp_port'})
  {
   ...
   $ftp->command("PRET", $cmd, @_)->response() if $ftp->supported("PRET");
...
sub _CPSV { shift->command("CPSV")->response()	 == CMD_OK }
sub _PROTP { shift->command("PROT P")->response() == CMD_OK }
sub _PROTC { shift->command("PROT C")->response() == CMD_OK }
sub _PBSZ0 { shift->command("PBSZ 0")->response() == CMD_OK }
